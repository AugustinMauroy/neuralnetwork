...
class NaturalLanguageClassifier {
    ...
    documents: { document: string; category: string }[];
    features: Set<string>;
    categories: Set<string>;
    categoryCount: Map<string, number>;
    featureCount: Map<string, number>;
    featureProbabilities: Map<string, number>;
    categoryProbabilities: Map<string, number>;
    ...
    addDocument(document: string, category: string): void {
    ...
    classify(document: string, context: string): string | string[] {
        ...
        this.categories.forEach(category => {
            ...
            let prob = this.categoryProbabilities.get(category) || 0;
            words.forEach(word => {

                const featureProbKey = `${word}:${category}`;
                if (this.featureProbabilities.has(featureProbKey)) {

                    prob *= this.featureProbabilities.get(featureProbKey) || 0;
    ...
    countFeaturesInCategory(category: string): number {
    ...
    countFeaturesNotInCategory(category: string): number {
    ...
    loadModelFromFile(filePath: string): void {
    ...
    saveModelToFile(filePath: string): void {
